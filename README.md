...
- **Precision**: when packaging a Parakeet model, `build.sh` asks `Model precision (int8/fp32)` — the default is auto-detected from whatever ONNX files are already in the model folder (int8 has `encoder-model.int8.onnx`, fp32 has `encoder-model.onnx` + `.data`). Choose openly; the script validates the folder contains the right files and warns about memory (fp32 ≈ 3GB VRAM/worker on GPU, ≈6GB RAM/worker on CPU). The selected precision is baked into the AppImage and shown in the startup banner.
